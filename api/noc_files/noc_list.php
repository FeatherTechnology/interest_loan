...
$query = "SELECT cc.id, cc.cus_id, cc.aadhar_number, CONCAT(cc.first_name, ' ', COALESCE(cc.last_name, '')) AS cus_name, anc.areaname, lnc.linename, bc.branch_name, cc.mobile1 ,cs.status, cs.id 
...
WHERE (cs.status = 12 OR cs.status = 13) AND u.id ='$user_id' AND us.id ='$user_id' ";
...
foreach ($result as $row) {
    ...
    if ($row['status'] == '13') {
        $action .= "<a href='#' id='remove-noc' value='" . $row['cus_id'] . "' title='Remove'>Remove</a>";
    }
