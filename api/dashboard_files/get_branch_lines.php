...
if ($branch_id == '0') {
    ...
    if ($qry->rowCount() > 0) {
        ...
        $qry1 = $pdo->query("SELECT line_id FROM `area_creation` WHERE branch_id IN ($branches) ");
        if ($qry1->rowCount() > 0) {
    ...
    $qry1 = $pdo->query("SELECT line_id FROM `area_creation` WHERE branch_id = $branch_id ");
    if ($qry1->rowCount() > 0) {
        while ($row = $qry1->fetch(PDO::FETCH_ASSOC)) {
            $result[] = $row['line_id'];
        }
