...
//Today Approval
$today_le = "SELECT COALESCE(count(le.id),0) AS today_approval FROM `loan_entry` le JOIN customer_creation cc ON cc.cus_id = le.cus_id JOIN customer_status cs ON le.id = cs.loan_entry_id WHERE cs.status >= 4  AND cs.status NOT IN (5, 6, 8, 9)  AND DATE(cs.updated_on) = CURDATE() ";
