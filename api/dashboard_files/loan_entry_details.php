...
//Total Loan Entry
$tot_le = "SELECT COALESCE(count(le.id),0) AS total_entry FROM `loan_entry` le JOIN customer_creation cc ON cc.cus_id = le.cus_id JOIN customer_status cs ON le.id = cs.loan_entry_id WHERE cs.status > 1 ";
...
//Today Loan Entry
$today_le = "SELECT COALESCE(count(le.id),0) AS today_entry FROM `loan_entry` le JOIN customer_creation cc ON cc.cus_id = le.cus_id JOIN customer_status cs ON le.id = cs.loan_entry_id WHERE cs.status > 1 AND DATE(le.created_on) = CURDATE() ";
