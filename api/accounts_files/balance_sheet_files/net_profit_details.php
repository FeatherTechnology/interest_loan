...
$qry = $pdo->query("SELECT  COALESCE(le.doc_charge_calculate, 0) AS doc_charges, COALESCE(le.processing_fees_calculate, 0) AS proc_charges FROM loan_issue li JOIN loan_entry le ON li.loan_entry_id = le.id WHERE $lelcwhere  GROUP BY le.id"); // Group by each loan entry
...
$qry2 = $pdo->query("SELECT COALESCE(SUM(interest_amount_track),0) AS interest, COALESCE(SUM(penalty_track),0) AS penalty, COALESCE(SUM(fine_charge_track),0) AS fine FROM `collection` WHERE $collwhere "); //Collection 
